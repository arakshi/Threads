...
class PublicWebCollector(BaseCollector):
    ...
    def _discover_from_account(self, handle: str) -> list[str]:
        ...
        try:
            ...
            found = set(re.findall(r'https://www\.threads\.(?:net|com)/@[^"\']+/post/[A-Za-z0-9_-]+', resp.text))
            if not found:
                found = set(re.findall(r'/@[^"\']+/post/[A-Za-z0-9_-]+', resp.text))
                found = {f'https://www.threads.com{x}' for x in found}
            return list(found)
    ...
    def collect(self, seeds: dict) -> list[dict]:
        ...
        records: list[dict] = []
        for url in {u.replace('threads.net','threads.com') for u in urls}:
            try:
