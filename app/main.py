...
def collect_ui(urls: str = '', keywords: str = 'продажи,маркетинг,бизнес', only_keywords: int = 0) -> RedirectResponse:
    post_urls = [u.strip() for u in urls.splitlines() if u.strip()]
    kw_list = [k.strip() for k in keywords.split(',') if k.strip()]
    db = SessionLocal()
    jobs = JobRepo(db)
    accounts = [] if only_keywords else [a.handle for a in AccountRepo(db).list()]
...
def local_ui(msg: str = '', show_guide: int = 0, q: str = '', min_score: float = 0.0) -> str:
    ...
    sum_likes = sum(p.likes for p in filtered if p.likes is not None)
    sum_replies = sum(p.replies for p in filtered if p.replies is not None)
    sum_reposts = sum(p.reposts for p in filtered if p.reposts is not None)
    sum_quotes = sum(p.quotes for p in filtered if p.quotes is not None)
    likes_cov = sum(1 for p in filtered if p.likes is not None)
    ...
    no_result_hint = "<div class='muted' style='margin:8px 0'>Ничего не найдено. Проверьте опечатку в фильтре или нажмите «Запустить сбор» ещё раз.</div>" if not filtered else ''

    def v(x):
        return x if x is not None else 'н/д'

    posts_rows = ''.join(
        f"<tr><td><a href='{escape(p.post_url)}' target='_blank'>открыть</a></td><td>{escape((p.text or '')[:180])}</td><td>{v(p.likes)}</td><td>{v(p.replies)}</td><td>{v(p.reposts)}</td><td>{v(p.quotes)}</td><td>{round(p.viral_score or 0, 2)}</td></tr>"
        for p in filtered
    ) or "<tr><td colspan='7'>Постов по фильтрам не найдено</td></tr>"
    ...
    <head>
      ...
      <style>
        ...
        .kpi {{ display:grid; grid-template-columns:repeat(8,1fr); gap:10px; }}
    ...
    <body>
      <div class='wrap'>
        ...
        <div class='notice'><b>Режим:</b> реальный сбор из открытого веба (не демо).</div>
        <div class='card'>
        ...
        <div class='kpi card'>
          ...
          <div class='pill'>Сумма лайков<br><b>{sum_likes}</b><br><span class='muted'>доступно у {likes_cov} постов</span></div>
          <div class='pill'>Сумма ответов<br><b>{sum_replies}</b></div>
          <div class='pill'>Сумма репостов<br><b>{sum_reposts}</b></div>
          <div class='pill'>Сумма цитат<br><b>{sum_quotes}</b></div>
        ...
        <div class='card'>
          <h2>Глобальный сбор по ключам (весь тредс)</h2>
          <form action='/app/collect' method='get'>
            <input name='keywords' value='маркетинг,продажи,бизнес' placeholder='ключевые слова через запятую'>
            <input type='hidden' name='only_keywords' value='1'>
            <button class='btn main' type='submit'>Запустить глобальный сбор</button>
          </form>
          <div class='muted' style='margin-top:8px'>Этот режим ищет посты по ключам по всему публичному вебу Threads, без привязки к вашим аккаунтам.</div>
        </div>
        ...
        <div class='card'>
          ...
          {no_result_hint}
          <table>
            <tr><th>Ссылка</th><th>Текст</th><th>Лайки</th><th>Ответы</th><th>Репосты</th><th>Цитаты</th><th>Вирусность</th></tr>
            {posts_rows}
          </table>
        </div>
