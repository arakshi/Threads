...
def local_ui(msg: str = '', show_guide: int = 0, q: str = '', min_score: float = 0.0) -> str:
    ...
    sum_replies = sum((p.replies or 0) for p in filtered)
    sum_reposts = sum((p.reposts or 0) for p in filtered)
    sum_quotes = sum((p.quotes or 0) for p in filtered)
    ...
    posts_rows = ''.join(
        f"<tr><td><a href='{escape(p.post_url)}' target='_blank'>открыть</a></td><td>{escape((p.text or '')[:180])}</td><td>{p.likes or 0}</td><td>{p.replies or 0}</td><td>{p.reposts or 0}</td><td>{p.quotes or 0}</td><td>{round(p.viral_score or 0, 2)}</td></tr>"
        for p in filtered
